...
// --- DigitalWaterCoin (DWC) kernel ----------------------------------------
// Identical SHA-256 core to rpow2, but the per-thread counter is written as 16
// ASCII hex bytes (it lives inside the UTF-8 string `address|epoch|nonce`) and
// difficulty counts *leading* zero bits.

constexpr std::uint32_t kDwcHexNonceLen = 16;

struct DwcKernelParams {
    std::uint32_t prefix_len;
    std::uint32_t difficulty_bits;
    std::uint32_t block_count;
    std::uint32_t _padding;
...
// Write the 64-bit counter as 16 ASCII hex chars (most-significant nibble
// first) starting at byte offset `prefix_len`, patching only the words that
// fall inside `block`.
inline void dwc_write_nonce_words(thread uint* w,
                                  uint block,
                                  uint prefix_len,
                                  ulong nonce) {
    for (uint i = 0; i < 16u; ++i) {
        ...
        const uint nibble = uint((nonce >> ((15u - i) * 4u)) & 0xful);
        const uint ascii = nibble < 10u ? (uint('0') + nibble) : (uint('a') + nibble - 10u);
        const uint block_byte = absolute_byte & 63u;
        const uint word_index = block_byte >> 2;
        const uint byte_index = block_byte & 3u;
        const uint shift = (3u - byte_index) * 8u;
...
inline uint dwc_leading_zero_bits(uint h0, uint h1, uint h2, uint h3,
    ...
    for (uint i = 0; i < 8u; ++i) {
        ...
        return bits + clz(word);
    }
    return bits;
}

kernel void dwc_mine_kernel(constant uint* template_words [[buffer(0)]],
                            constant DwcKernelParams& params [[buffer(1)]],
                            device atomic_uint* found [[buffer(2)]],
                            device ulong* found_nonce [[buffer(3)]],
                            device uint* found_digest [[buffer(4)]],
                            uint gid [[thread_position_in_grid]]) {
    ...
    for (uint block = 0; block < params.block_count; ++block) {
        ...
        dwc_write_nonce_words(w, block, params.prefix_len, nonce);
        for (uint i = 16; i < 64; ++i) {
            const uint s0 = rotate(w[i - 15], 25u) ^ rotate(w[i - 15], 14u) ^ (w[i - 15] >> 3);
            const uint s1 = rotate(w[i - 2], 15u) ^ rotate(w[i - 2], 13u) ^ (w[i - 2] >> 10);
            w[i] = w[i - 16] + s0 + w[i - 7] + s1;
    ...
    if (dwc_leading_zero_bits(h0, h1, h2, h3, h4, h5, h6, h7) >= params.difficulty_bits) {
        if (atomic_exchange_explicit(found, 1u, memory_order_relaxed) == 0u) {
            *found_nonce = nonce;
            found_digest[0] = h0; found_digest[1] = h1;
            found_digest[2] = h2; found_digest[3] = h3;
            found_digest[4] = h4; found_digest[5] = h5;
            found_digest[6] = h6; found_digest[7] = h7;
        }
    }
}
)METAL";
...
id<MTLComputePipelineState> dwc_pipeline_for_device(id<MTLDevice> device) {
    static NSMutableDictionary<NSNumber*, id<MTLComputePipelineState>>* pipelines = nil;
    if (pipelines == nil) {
        pipelines = [NSMutableDictionary dictionary];
    }
    ...
    NSError* error = nil;
    NSString* source = [NSString stringWithUTF8String:dwc_kernel_source()];
    id<MTLLibrary> library = [device newLibraryWithSource:source options:nil error:&error];
    if (library == nil) {
        throw std::runtime_error(std::string("Metal DWC kernel compile failed: ")
            + (error.localizedDescription.UTF8String ?: "unknown error"));
    }
    id<MTLFunction> function = [library newFunctionWithName:@"dwc_mine_kernel"];
    if (function == nil) {
        throw std::runtime_error("Metal DWC kernel function not found");
    }
    id<MTLComputePipelineState> pipeline = [device newComputePipelineStateWithFunction:function error:&error];
    if (pipeline == nil) {
        throw std::runtime_error(std::string("Metal DWC pipeline creation failed: ")
            + (error.localizedDescription.UTF8String ?: "unknown error"));
    }
    pipelines[key] = pipeline;
    return pipeline;
...
std::vector<std::uint32_t> build_dwc_template_words(const mining_metal_dwc_job& job) {
    const auto message_len = job.prefix_len + kDwcHexNonceLen;
    const auto padded_len = ((message_len + 9 + 63) / 64) * 64;
    if (padded_len > 128) {
        throw std::runtime_error("DWC padded message is too long for Metal solver");
    }

    std::array<std::uint8_t, 128> padded{};
    if (job.prefix_len > 0) {
        std::memcpy(padded.data(), job.prefix_ptr, job.prefix_len);
    }
    padded[message_len] = 0x80;
    const auto bit_len = static_cast<std::uint64_t>(message_len) * 8;
    for (std::size_t i = 0; i < 8; ++i) {
...
void fill_dwc_result(bool found,
                     std::uint64_t nonce,
                     std::int64_t attempts,
                     const std::uint32_t digest_words[8],
...
struct mining_metal_dwc_session {
    ...
    id<MTLBuffer> nonce_buffer;
    id<MTLBuffer> digest_buffer;
    DwcKernelParams params{};
    std::uint64_t next_nonce = 0;
    std::int64_t attempts = 0;

    mining_metal_dwc_session(std::size_t device_index,
                             const mining_metal_dwc_job& job,
                             const mining_metal_dwc_solver_config& config,
                             std::uint64_t start_nonce) {
        if (job.prefix_ptr == nullptr && job.prefix_len > 0) {
            throw std::runtime_error("DWC prefix pointer is null");
        }
        if (job.prefix_len > 103) {
            throw std::runtime_error("DWC prefix is too long for Metal solver");
        }
        if (config.batch_size == 0) {
            throw std::runtime_error("DWC Metal batch size must be greater than zero");
        }
        if (config.batch_size > std::numeric_limits<std::uint32_t>::max()) {
            throw std::runtime_error("DWC Metal batch size exceeds kernel grid limit");
        }
        ...
        id<MTLDevice> device = devices[device_index];
        pipeline = dwc_pipeline_for_device(device);
        queue = [device newCommandQueue];
        if (queue == nil) {
            throw std::runtime_error("failed to create Metal command queue");
        }

        const auto template_words = build_dwc_template_words(job);
        template_buffer = [device newBufferWithBytes:template_words.data()
                                              length:template_words.size() * sizeof(std::uint32_t)
                                             options:MTLResourceStorageModeShared];
        params = DwcKernelParams{
            .prefix_len = static_cast<std::uint32_t>(job.prefix_len),
            .difficulty_bits = job.difficulty_bits,
            .block_count = static_cast<std::uint32_t>(template_words.size() / 16),
            ._padding = 0,
            .start_nonce = start_nonce,
        ...
        if (template_buffer == nil || params_buffer == nil || found_buffer == nil ||
            nonce_buffer == nil || digest_buffer == nil) {
            throw std::runtime_error("failed to allocate Metal DWC buffers");
        }
        next_nonce = start_nonce;
    }

    void mine_next_batch(mining_metal_dwc_mine_result* result) {
        if (std::numeric_limits<std::uint64_t>::max() - next_nonce < params.batch_size) {
            throw std::runtime_error("DWC Metal nonce range exhausted");
        }
        ...
        *static_cast<std::uint64_t*>(nonce_buffer.contents) = 0;
        ...
        [command_buffer waitUntilCompleted];
        if (command_buffer.error != nil) {
            throw std::runtime_error(std::string("Metal DWC command failed: ")
                + (command_buffer.error.localizedDescription.UTF8String ?: "unknown error"));
        }
        ...
        attempts = static_cast<std::int64_t>(std::min<std::uint64_t>(
            static_cast<std::uint64_t>(std::numeric_limits<std::int64_t>::max()),
            static_cast<std::uint64_t>(attempts) + params.batch_size));
        fill_dwc_result(*found_ptr != 0, *nonce_ptr, attempts, digest_ptr, result);
        next_nonce += params.batch_size;
    }
};
...
bool mining_metal_dwc_is_available() {
    return mining_metal_is_available();
}

mining_metal_dwc_session* mining_metal_dwc_session_create(
    std::size_t device_index,
    const mining_metal_dwc_job* job,
    const mining_metal_dwc_solver_config* config,
    std::uint64_t start_nonce) {
    ...
    try {
        clear_last_error();
        return new mining_metal_dwc_session(device_index, *job, *config, start_nonce);
    } catch (const std::exception& error) {
        set_last_error(error);
        return nullptr;
    }
...
bool mining_metal_dwc_session_mine_next_batch(
...
void mining_metal_dwc_session_destroy(mining_metal_dwc_session* session) {
    delete session;
}
